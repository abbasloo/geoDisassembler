...
main (int argc, char** argv)
{
  ...
  // Files
  std::ofstream myfile;
  myfile.open ((std::string) argv[6] + "SegCon.csv");
  std::ofstream myfilee;
  myfilee.open ((std::string) argv[6] + "Seg.csv");
  std::ofstream myfileee;
  myfileee.open ((std::string) argv[6] + "Feat.csv");
  std::ofstream myfileeee;
  myfileeee.open ((std::string) argv[6] + "Ext.csv");
  ...
  for (size_t counter = 0; counter < clusters.size (); ++counter)
  {
	  ...
	  mlsHere.setSearchRadius (0.1);
	  mlsHere.process (mlsHere_points);

	  //std::cout << std::endl;
	  std::string filename = "../data/pc/" + (std::string) argv[6] + "_seg_" + std::to_string(counter) +".pcd";
	  ...
	  nHere.compute (*normalsHere);
          ...
          // Point cloud signature. Create the pfh estimator, and pass the cloud and normals to it
	  ...
	  // Saving the result
	  std::string filenamevtk = "../data/mesh/" + (std::string) argv[6] + "_seg_" + std::to_string(counter) +".vtk";
	  std::string filenameobj = "../data/mesh/" + (std::string) argv[6] + "_seg_" + std::to_string(counter) +".obj";
	  pcl::io::saveVTKFile (filenamevtk, trianglesHere);
	  pcl::io::saveOBJFile (filenameobj, trianglesHere);
  }
  writer.write ("../data/pc/" + (std::string) argv[6] + "_seg.pcd", *colored_cloud, false);
  myfile.close();
  myfilee.close();
  myfileee.close();
  myfileeee.close();		

  // Saving the difference. The point that are in very small segments
  float resolution = 0.001f;	
  pcl::octree::OctreePointCloudChangeDetector<pcl::PointXYZ> octree (resolution);
  octree.setInputCloud (cloudSeg);
  ...
  cloudDiff->width = newPointIdxVector.size ();
  cloudDiff->height = 1;
  cloudDiff->points.resize (cloudDiff->width * cloudDiff->height);
  for (size_t i = 0; i < newPointIdxVector.size (); ++i)
  { 
        cloudDiff->points[i].x = cloud->points[newPointIdxVector[i]].x;
	cloudDiff->points[i].y = cloud->points[newPointIdxVector[i]].y;
        cloudDiff->points[i].z = cloud->points[newPointIdxVector[i]].z;
  }

  pcl::PointCloud<pcl::PointNormal> mlsDiff_points;
  pcl::MovingLeastSquares<pcl::PointXYZ, pcl::PointNormal> mlsDiff;
  pcl::search::KdTree<pcl::PointXYZ>::Ptr treeDiff (new pcl::search::KdTree<pcl::PointXYZ>);
  mlsDiff.setComputeNormals (true);
  mlsDiff.setInputCloud (cloudDiff);
  mlsDiff.setPolynomialFit (true);
  mlsDiff.setSearchMethod (treeDiff);
  mlsDiff.setSearchRadius (0.1);
  mlsDiff.process (mlsDiff_points);
  writer.write ("../data/pc/" + (std::string) argv[6] + "_diff.pcd", mlsDiff_points, false);
  ...
  // Saving the result
  pcl::io::saveVTKFile ("../data/mesh/" + (std::string) argv[6] + "_seg.vtk", triangles);
  pcl::io::saveOBJFile ("../data/mesh/" + (std::string) argv[6] + "_seg.obj", triangles);
