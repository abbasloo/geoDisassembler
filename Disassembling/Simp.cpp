	      ...
	      int t = 0;
			  ...
			  if (val <= 1e-3 && a != 0.0 && T != 0.0) {
				  ...
				  if (doIt) {cloud_g->push_back(pp); t += 1;}
                          ...
                          float T = (float) (rand() % scale + 1)/100.0; T *= std::atof(argv[2]);
			  pcl::PointXYZ pp;  
			  pp.x = cloud->points[i].x + T*principal_curvatures->points[i].principal_curvature_x;
			  pp.y = cloud->points[i].y + T*principal_curvatures->points[i].principal_curvature_y;
			  pp.z = cloud->points[i].z + T*principal_curvatures->points[i].principal_curvature_z; 
