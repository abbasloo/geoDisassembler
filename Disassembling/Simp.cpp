...
main (int argc, char** argv)
{
  ...
  */
   ...
   if (std::atof(argv[6]) == 1) {
	  ...
	  //vg.setLeafSize (std::atof(argv[2])/2.0, std::atof(argv[2])/2.0, std::atof(argv[2])/2.0);
	  ...
	  nMesh.compute (*normalsMesh);
          ...
          pcl::concatenateFields (*cloud, *normalsMesh, *cloud_with_normals);
	  ...
	  for (size_t i = 0; i < cloud->points.size (); ++i)
	  {
		  ...
		  float T = (float) (rand() % scale + 1); T *= std::atof(argv[2]);
                  float a = std::sqrt(t1*t1 + t2*t2 + t3*t3);
                  t1 /= a; t2 /= a; t3 /= a;
		  float val = std::abs(cloud_with_normals->points[i].normal_x*T*t1 + cloud_with_normals->points[i].normal_y*T*t2 + cloud_with_normals->points[i].normal_z*T*t3); 
		  if (val <= 1e-3 && a != 0.0 && T != 0.0) {
                          //std::cout << t1 << ',' << t2 << ',' << t3 << ',' << T << std::endl;
                          //std::cout << cloud_with_normals->points[i].normal_x << ',' << cloud_with_normals->points[i].normal_y << ',' << cloud_with_normals->points[i].normal_z << std::endl;
			  pcl::PointXYZ pp;  
