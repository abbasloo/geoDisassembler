...
Feat = []
with open('../Segmentation/' + test + 'Feat.csv', 'rb') as csvfile:
...
con = [[]]
with open('../Segmentation/' + test + 'SegCon.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=';')
     for row in spamreader:
	 con_ = []
	 for i in range(len(row)):
...
for i in range(len(con[ID])):
	if i == 0:
		seg += ' ../data/mesh/' + test + '_seg_' + np.str(np.int(con[ID][i])) + '.vtk'
	if i != 0:
		seg += ' ../data/pc/' + test + '_seg_' + np.str(np.int(con[ID][i])) + '.pcd'
...
ar7 = []
ar8 = []
with open('../Segmentation/' + test + 'Seg.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=';')
     for row in spamreader:
...
for i in range(len(a)):
	seg += ' ../data/pc/done/' + test + '_seg_' + np.str(a[i]) + '.pcd'
	d += '"' + np.str(np.int(a[i])) + '" '
for i in range(len(b)):
	seg += ' ../data/pc/done/' + test + '_seg_' + np.str(b[i]) + '.pcd'
	d += '"' + np.str(np.int(b[i])) + '" '
...
con = []
with open('../Segmentation/' + test + 'Ext.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=';')
     for row in spamreader:
...
for i in range(len(con)):
	...
	#		doIt = False
		...
		if doIt:
	        	seg += ' ../data/pc/' + test + '_seg_' + np.str(np.int(con[i])) + '.pcd'
...
for i in range(len(idx_456)):
        ...
        if (ar8[i] > 10 and  doIt):
	        seg += ' ../data/pc/' + test + '_seg_' + np.str(i) + '.pcd'
#os.system('pcl_viewer ' + seg)
...
for i in range(len(con)):
	doIt = True
        ...
        if (ar8[np.int(con[i])] > 100 and idx_456[np.int(con[i])]):
		if doIt:
	        	seg += ' ../data/pc/done/' + test + '_seg_' + np.str(np.int(con[i])) + '.pcd'
                        d += '"' + np.str(np.int(con[i])) + '" '
#print (d)
#os.system('pcl_viewer ' + seg)
